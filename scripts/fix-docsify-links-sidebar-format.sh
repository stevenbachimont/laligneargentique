...
# Script pour corriger les liens Markdown dans la documentation Docsify (Format Sidebar)
# Usage: ./scripts/fix-docsify-links-sidebar-format.sh

set -e

echo "🔧 Correction des liens Docsify dans la documentation (Format Sidebar)..."
...
fix_links_in_file() {
    ...
    if [ -f "$file" ]; then
        ...
        # Remplacer les liens relatifs par des liens Docsify avec format sidebar
        sed -i '' "s|\.\/\([^)]*\.md\)|#/$section_dir/\1|g" "$file"
        sed -i '' "s|\.\.\/\([^)]*\.md\)|#/$section_dir/\1|g" "$file"
        
        # Supprimer l'extension .md et garder les majuscules (format sidebar)
...
echo "   - Conservation des majuscules (format sidebar)"
echo "   - Format identique à la sidebar qui fonctionne"
echo ""
