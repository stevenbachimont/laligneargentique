...
# Script pour corriger les liens Markdown dans la documentation Docsify
# Usage: ./scripts/fix-docsify-links.sh

set -e

echo "🔧 Correction des liens Docsify dans la documentation..."
...
fix_links_in_file() {
    ...
    if [ -f "$file" ]; then
        ...
        # Remplacer les liens relatifs par des liens Docsify
        sed -i '' "s|\.\/\([^)]*\.md\)|#/$section_dir/\1|g" "$file"
        sed -i '' "s|\.\.\/\([^)]*\.md\)|#/$section_dir/\1|g" "$file"
        
        # Supprimer l'extension .md des liens Docsify
